...
df_TypeI_omni <- sig_omnibus(p_omni, 0.05)  %>% 
  select(-c(alpha, N_sig, effnames, p.value, sig, omniF)) %>% 
  distinct() %>% 
...
fig_simu_a <- df_omni_tmp %>% 
  ...
  scale_fill_manual(values = custom_clr[c(1,5)],
                    breaks = levels(df_omni_tmp$adjust)) + #,  
...
fig_simu_a_supp <- bind_rows(df_wo_omni, df_w_omni) %>% 
...
df_posthoc_tmp_supp <- df_TypeI_posthoc %>% 
  pivot_longer(starts_with("TypeI_"), names_to = "effects", values_to = "TypeI") %>% 
  mutate(adjust = if_else(effects == "TypeI_main", "uncorrected", 
                          if_else(adjust == "uncorrected", "uncorrected", 
                                  str_to_title(as.character(adjust)))),
...
for (i in unique(df_posthoc_tmp_supp$N_level)) {
  ...
  assign(
    ...
    labs(x = paste0("A main effect with a factor of ", i, " levels"), y = "Type I Error Rate (%)", fill = "") +
    theme(legend.position = c(0.75, 0.6),
          axis.ticks.length = unit(.15, "cm"),
          axis.text.x = element_blank(),
          panel.spacing.x = unit(0, "pt"),
