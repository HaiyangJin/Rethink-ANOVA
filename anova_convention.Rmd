...
df_TypeI_omni <- sig_omnibus(p_omni, 0.05) %>% 
  ...
  summarize(TypeI_omniF = mean(sig_omniF), 
            ...
            `TypeI_omniF&any` = mean(sig_with_omni),
            .groups = "drop") 
...
# ggsave(file.path("images","simu_mainpost.png"), fig_simu_a, width = 8, height = 5.6)
fig_simu_a
```

```{r fig.width=13, fig.asp=0.4}

# df_omni_cber_supp <- df_TypeI_omni %>% 
#   pivot_longer(starts_with("TypeI_"), names_to = "effnames", values_to = "TypeI") %>% 
#   mutate(adjust = if_else(effnames == "TypeI_omniF", "uncorrected", adjust),
#          adjust = fct_relevel(adjust, "uncorrected"),
#          effnames = factor(effnames, 
...
for (i in unique(df_omni_cber$N_IV)) {
  ...
  assign(
    nam, df_omni_cber %>% 
      ...
      labs(x = paste0("An ANOVA with ", i, " factors"), y = "Type I Error Rate (%)", fill = "") +
      theme(legend.position = c(0.75, 0.6),
            axis.ticks.length = unit(.15, "cm"),
            axis.text.x = element_blank(),
            panel.spacing.x = unit(0, "pt"),
...
# ggsave(file.path("images","simu_mainpost.png"), fig_simu_a_supp, width = 8, height = 5.6)
fig_simu_a_supp
```
...
df_TypeI_single1 <- sig_omnibus(p_omni, 0.05)  %>% 
  filter(adjust == "uncorrected") %>% 
  mutate(sig_omni_single = sig_omniF & sig) %>% # omni and one specific effect
  select(iter, N_IV, effnames, sig_with_omni, sig_omni_single) %>% 
  group_by(N_IV, effnames) %>% 
  summarize(TypeI_cber = mean(sig_with_omni), # omni and at least one sig
            TypeI_single = mean(sig_omni_single),
            .groups = "drop") %>% 
  mutate(Method = "omnibus F-test")

df_TypeI_single2 <- sig_omnibus(p_omni, 0.05)  %>% 
  filter(adjust == "Bonferroni") %>% 
  mutate(sig_any_single = sig_any & sig) %>% 
  select(iter, N_IV, effnames, sig_any, sig_any_single) %>% 
  group_by(N_IV, effnames) %>% 
  summarize(TypeI_cber = mean(sig_any), 
            TypeI_single = mean(sig_any_single),
            .groups = "drop") %>% 
  mutate(Method = "Bonferroni")

df_TypeI_single <- bind_rows(df_TypeI_single1, df_TypeI_single2) 

# Type I error rate for single tests (when the FWER is controlled)
df_TypeI_single %>%
  distinct() %>% 
  pivot_wider(names_from = effnames, values_from = TypeI_single) %>% 
  select(Method, N_IV, TypeI_cber, everything())
```


```{r fig.width=4, fig.asp=1.2}
fig_simu_a_single <- df_TypeI_single %>% 
  filter(N_IV == 2) %>% 
  select(-c(N_IV)) %>% 
  pivot_wider(c(Method, TypeI_cber), 
              names_from = effnames, values_from = TypeI_single) %>% 
  pivot_longer(!("Method"), 
...
# ggsave(file.path("images","simu_omnibus_single.png"), fig_simu_a_single, width = 8, height = 4.8)
fig_simu_a_single
```

```{r fig.width=8, fig.asp=0.8}
df_TypeI_single_plot <- df_TypeI_single %>% 
  pivot_wider(c(Method, N_IV, TypeI_cber), 
              names_from = effnames, values_from = TypeI_single) %>% 
  pivot_longer(!c("Method", "N_IV"), 
               names_to = "effnames", values_to = "TypeI") %>% 
  mutate(effnames = factor(effnames),
         effnames = fct_relevel(effnames, "TypeI_cber"),
         N_IV = factor(N_IV),
         N_IV = fct_recode(N_IV, `An ANOVA with two factors`="2",
                           `An ANOVA with three factors`="3",
                           `An ANOVA with four factors`="4",
                           `An ANOVA with five factors`="5"),
         Method = as_factor(Method)) %>% 
  filter(!is.na(TypeI))
  
fig_simu_single_supp1 <- df_TypeI_single_plot %>% 
  filter(N_IV %in% c("An ANOVA with two factors", "An ANOVA with three factors")) %>% 
  ggplot(aes(x = Method, y = TypeI*100, fill = effnames, group = effnames)) +
  geom_col(position = "dodge") +
...
fig_simu_single_supp2 <- df_TypeI_single_plot %>% 
  filter(N_IV %in% c("An ANOVA with four factors", "An ANOVA with five factors")) %>% 
  ggplot(aes(x = Method, y = TypeI*100, fill = effnames, group = effnames)) +
  geom_col(position = "dodge") +
  facet_wrap(vars(N_IV), switch = "x") +
  scale_fill_manual(values = c(custom_clr[c(1,3)], brewer.pal(8,"Set1"), 
...
fig_simu_single_supp <- ggarrange(fig_simu_single_supp1, fig_simu_single_supp2,
                                  nrow=2)

# ggsave(file.path("images","simu_omnibus_single_supp.png"), fig_simu_single_supp, width = 8, height = 4.8)
fig_simu_single_supp
```
...
for (i in unique(df_posthoc_tmp$N_level)) {
  ...
  assign(
    ...
    labs(x = paste0("A main effect with ", i, " levels"), y = "Type I Error Rate (%)", fill = "") +
    theme(legend.position = c(0.75, 0.6),
          axis.ticks.length = unit(.15, "cm"),
          axis.text.x = element_blank(),
          panel.spacing.x = unit(0, "pt"),
...
# ggsave(file.path("images","simu_mainpost.png"), fig_simu_b_supp, width = 8, height = 5.6)
fig_simu_b_supp
```
...
```{r}
# with omnibus F-test and significant effect
df_TypeI_main_single1 <- sig_main_posthoc(p_main_posthoc, 0.05) %>% 
  filter(adjust == "uncorrected") %>% 
  mutate(sig_main_single = sig_main & sig) %>% # main and one pairwise comparison
  select(iter, N_level, contrast, `sig_main&posthoc`, sig_main_single) %>% 
  group_by(N_level, contrast) %>% 
  summarize(TypeI_cber = mean(`sig_main&posthoc`), # main and at least one sig
            TypeI_single = mean(sig_main_single),
            .groups = "drop") %>% 
  mutate(Method = "Main effect & Post-hoc")

df_TypeI_main_single2 <- sig_main_posthoc(p_main_posthoc, 0.05) %>% 
  filter(adjust != "uncorrected") %>% 
  mutate(sig_any_single = sig_anypost & sig, # correction and one specific effect
         Method=str_to_title(adjust)) %>% 
  select(iter, N_level, contrast, Method, sig_anypost, sig_any_single) %>% 
  group_by(N_level, contrast, Method) %>% 
  summarize(TypeI_cber = mean(sig_anypost), # correction and at least one sig
            TypeI_single = mean(sig_any_single),
            .groups = "drop") 

df_TypeI_main_single <- bind_rows(df_TypeI_main_single1, df_TypeI_main_single2) 

# Type I error rate for single tests (when the FWER is controlled)
df_TypeI_main_single %>%
  pivot_wider(names_from = contrast, values_from = TypeI_single) %>% 
  select(Method, N_level, TypeI_cber, everything())
```

```{r fig.width=12, fig.asp=0.4}
fig_simu_b_single <- df_TypeI_main_single %>% 
  filter(N_level == 4) %>% 
  select(-c(N_level)) %>% 
  pivot_wider(c(Method, TypeI_cber), 
              names_from = contrast, values_from = TypeI_single) %>% 
  pivot_longer(!("Method"), 
...
# ggsave(file.path("images","simu_main_single.png"), fig_simu_b_single, width = 8, height = 4.8)
fig_simu_b_single
```
...
for (i in unique(df_TypeI_main_single$N_level)) {
  ...
  nam <- paste("fig_simu_b_single_tmp", i, sep = "")
...
fig_main_single_supp <- ggarrange(fig_simu_b_single_tmp3, 
                                  fig_simu_b_single_tmp4,
                                  fig_simu_b_single_tmp5,
                                  fig_simu_b_single_tmp6,
                                  ncol=1)

# ggsave(file.path("images","simu_main_single.png"), fig_simu_b_single, width = 8, height = 4.8)
fig_main_single_supp
```
...
df_inter_simp_tmp <- df_TypeI_inter %>% 
  ...
  mutate(adjust = if_else(adjust == "uncorrected", "uncorrected",
         ...
         effects = fct_recode(effects, 
                              `Simple effect analysis\n(two tests, FWER)`= "any_simple_byA_2", 
                              `Simple effect analysis\n(four tests, FWER)`="any_simple_byAB_4",
                              `Interaction & simple effects\nwith two tests (CER)`="inter&any_byA_2",
                              `Interaction & simple effects\nwith four tests (CER)`= "inter&any_byAB_4"))
...
df_inter_simp_tmp_supp <- df_TypeI_inter %>% 
  ...
  mutate(adjust = if_else(adjust == "uncorrected", "uncorrected",
         ...
         effects = factor(effects, 
                          levels = c("Interaction\n", 
                                     "any_simple_byB_2", "any_simple_byAB_4",
                                     "inter&any_byB_2", "inter&any_byAB_4")),
         effects = fct_recode(effects, 
                              `Simple effect analysis\n(two pairwise comparisons)`= "any_simple_byB_2", 
                              `Simple effect analysis\n(four pairwise comparisons)`="any_simple_byAB_4",
                              `Conjunction Type I Error Rate\n(interaction & two comparisons)`="inter&any_byB_2",
...
# ggsave(file.path("images","simu_mainpost.png"), fig_simu_c_supp, width = 8, height = 5.6)
fig_simu_c_supp
```
...
```{r fig.width=10.5, fig.asp=0.4}

# fig_simu_single <- 
  ggarrange(fig_simu_a_single, fig_simu_b_single,
          nrow = 1, labels = c("a", "b"),
          widths = c(0.3, 0.7))
```
