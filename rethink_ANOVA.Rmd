...
simu_control <- mvtnorm::rmvnorm(N, c(mean_pre, delta[1]), sigma = C*sd^2)
simu_protocal1 <- mvtnorm::rmvnorm(N, c(mean_pre, delta[2]), sigma = C*sd^2)
simu_protocal2 <- mvtnorm::rmvnorm(N, c(mean_pre, delta[3]), sigma = C*sd^2)
...
simu_null <- function(iter=100, N=30, rho=.5, sd=1, n_core=2,
  ...
  simu_null_single <- function(N_s=N, rho_s=rho, sd_s=sd){
    library(tidyverse)
    library(emmeans)
    ...
    simu_null3 <- mvtnorm::rmvnorm(N_s, mean, sigma = C*sd_s^2) %>% 
  ...
  if (!is.null(file_cache) && file.exists(as.character(file_cache))){
    ...
    Ns <- rep(N, times=iter)
    ls_tibble <- pbapply::pblapply(Ns, simu_null_single, cl = n_core)
...
simu_N <- simu_null(iter=5000, N=Ns, n_core = 18,
                    file_cache = here::here("simulation", "simu_N.rds"))
simu_alpha(simu_N, alpha = .1)
```
