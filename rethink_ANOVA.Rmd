...
train_sim <- contrast(train_emm, method = "revpairwise", by="Group")]
```

```{r simu-simple}
train_sim[2:3] %>% 
  as_tibble() %>% 
  apa_table(caption="Results of simple effects comparing pre- and post-tests for the two training groups respectively.")
```

These results (Table \@ref(tab:simu-simple)) showed that the post-test performance is better than the pre-test in both training groups. Nevertheless, we should not only use these effects to draw conclusions for the three questions. Then We conduct the simple interaction analysis:

```{r echo=TRUE}
train_inter <- contrast(train_emm, interaction="revpairwise")
```

```{r simu-inter}
train_inter %>% 
  as_tibble() %>% 
  apa_table(caption="Results of simple interaction analysis in the training study.")
```

With the results of both the simple effects and the simple interaction effects (Table \@ref(tab:simu-inter)), we may claim that:
...
```{r echo=TRUE}
train_custom <- contrast(train_emm, method = list("average training"=c(1, -1, -.5, .5, -.5, .5)))
```

```{r simu-custom}
train_custom %>% 
  as_tibble() %>% 
  apa_table(caption="Results of contrasting the average training increases with the control group.")
```

Results (Table \@ref(tab:simu-custom)) showed that the average training performance increases are higher than the control group.
...
```
```{r cf-lmm-fixed}
apa_table(summary(cf_lmm)$coefficients,
          caption='Fixed effect results in LMM for analyzing the composite face effect.',
          note="'Congruencyinc' examines how much the performance on aligned incongruent trails is larger than that on aligned congruent trials, which is significantly smaller than 0. 'Congruencyinc:Alignmentmis' examines the how much the Congruency effect in the aligned condition is larger than that in the misaligned condition, which is significantly larger than 0. These results indicated that composite face effect was observed in this study. Satterthwaite's method was used to calculate the results.")  # only show results of fixed effects
```

Results of fixed effects are displayed in Table \@ref(tab:cf-lmm-fixed). With the default dummy coding in R, `Congruencyinc` examines how much the performance on aligned incongruent trails is larger than that on aligned congruent trials, which is significantly smaller than 0. `Congruencyinc:Alignmentmis` examines the how much the Congruency effect in the aligned condition is larger than that in the misaligned condition, which is significantly larger than 0. In other words, the Congruency effect in the aligned condition is larger than 0, and it is also larger than that in the misaligned condition. With this evidence, we may claim the observation of the composite effect in this study. Note that since our predictions are directional, we may use one-sided, instead of two-sided, tests. For instance, we may use t-value of 1.65 instead of 1.96 as the threshold to claim the significant effect.
