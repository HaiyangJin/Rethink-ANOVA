...
# A fake training study

In this fake study, two protocols are proposed to train participants' face recognition abilities. Suppose we would like to know (1) whether the first protocol is effective, (2) whether the second protocol is effective, and (3) which protocol is more effective. Two groups of participants are trained with Protocol 1 and 2, respectively, and a third group completes a control task. All participants' performance in recognizing faces is measured before and after the training (or the control task). In summary, it is a 3 (Group: control, Protocol 1, vs. Protocol 2) × 2 (Test: pre-test vs. post-test) mixed experimental design. 
...
Let us simulate a data set for the above design with 30 participants for each group. Please refer to the open materials for detailed simulation codes. The "true" condition means are displayed in Table \@ref(tab:simu-true), and the structure of the simulated data is as follows:
...
We may check the results of the interaction from above ANOVA table. As ANOVAs obscure the direction of the effects, we need to additionally check the directional information. Alternatively, the directional information could be obtained by:
...
With the default dummy coding in R, `Congruencyinc` examines how much the performance on aligned incongruent trails is larger than that on aligned congruent trials, which is significantly smaller than 0. `Congruencyinc:Alignmentmis` examines the how much the Congruency effect in the aligned condition is larger than that in the misaligned condition, which is significantly larger than 0. In other words, the Congruency effect in the aligned condition is larger than 0, and it is also larger than that in the misaligned condition. With this evidence, we may claim the observation of the composite effect in this study. Note that since our predictions are directional, we may use one-sided, instead of two-sided, tests. For instance, we may use t-value of 1.65 instead of 1.96 as the threshold to claim the significant effect.
