...
params_sn <- list(
  ...
  # set the variances for lmm (std)
  var_residual = 100,  # residual
  var_rnd_int_subj = 100,  # random intercept for Subject
  var_rnd_int_stim = 100,  # random intercept for Stimuli
  var_rnd_slp_IV1_subj = 100,  # random slope of IV1 for Subject
  var_rnd_slp_IV2_subj = 100,  # random slope of IV2 for Subject
  var_rnd_slp_inter_subj = 20,  # random slope of IV1*IV2 for Subject
  var_rnd_slp_IV1_stim = 100,  # random slope of IV1 for Stimuli
  var_rnd_slp_IV2_stim = 100,  # random slope of IV2 for Stimuli
  var_rnd_slp_inter_stim = 20  # random slope of IV1*IV2 for Stimuli
)
...
rbind(mutate(as_tibble(emm_simu_ss),
  ...
  geom_text(aes(label=ptext, x=x, y=y), data=df_simp,
            color="black", size = 3, show.legend = F, parse=T) +
  labs(x="Orientation", y="Response times (ms)", color="Format", linetype="Format") +
  theme(legend.position=c(.5, .5)) +
  NULL

# ggsave(file="inter_simple_sig.png", width = 8, height = 6)
```
...
rbind(mutate(as_tibble(emm_simu_ss),
  ...
  geom_text(aes(label=labeltext_inter), data=df_simp, x=.5, y = 650,
            color="black", size = 3, hjust=0, show.legend = F, parse=T) +
  geom_text(aes(label=labeltext_sim), data=df_simp, x=.5, y = 630,
            color="black", size = 3, hjust=0, show.legend = F, parse=T) +
  geom_text(aes(label=ptext, x=x, y=y), data=df_inter,
            color="black", size = 3, show.legend = F, parse=T) +
  geom_text(aes(label=ptext, x=x, y=y), data=df_simp,
