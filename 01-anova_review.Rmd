...
This first section is the analysis of reviewing ANOVA prevalence and complexity, as well as the prevalence of ANOVA conventions in research articles published in six psychology journals in 2021: 
...
# Usage ANOVA
kappa_res <- tibble(
  Colname = "Research article",
  method = "Unweighted",
  value = Ka_res$Unweighted["value"],
  ASE = Ka_res$Unweighted["ASE"],
  lwr = confint(Ka_res)[1,1],
  upr = confint(Ka_res)[1,2],
  conf_mat = list(as.matrix(column_to_rownames(df_res_K, "Research article_YJ")))
)

kappa_res  # Kappa for research articles
```
...
kappa_aov <- tibble(
  Colname = "Use ANOVA",
  method = "Unweighted",
  value = Ka_aov$Unweighted["value"],
  ASE = Ka_aov$Unweighted["ASE"],
  lwr = confint(Ka_aov)[1,1],
  upr = confint(Ka_aov)[1,2],
  conf_mat = list(as.matrix(column_to_rownames(df_aov_K, "Use ANOVA_YJ")))
)

kappa_aov # Kappa for using ANOVA
```
...
```{r}
(kappa_df <- bind_rows(kappa_res, kappa_aov, kappa_ed, kappa_a, kappa_b, kappa_c, kappa_de, kappa_o))
```
...
<br>
The percentage of included articles (relative to the total number of articles):
```{r}
...
#### Alluvial figure 
Here is the information used to created the Alluvial figure at https://sankeymatic.com/build/ for displaying the ANOVA complexity.
```{r}
...
Convention A was only inspected in multi-way ANOVAs. 
```{r}
df_conv_a <- df_conv %>%
...
We recorded whether all the effects, including (at least statistically significant) main effects and interactions, were reported. 
...
df_prev_a <- df_conv_a_article %>%
  ...
  summarize(conA = mean(a1_per_article), 
            countA_arti = sum(a1_per_article>0.5), # more than half
            countA=n()) %>%
...
df_prev_a
```
`conA`: the prevalence of Convention A, which was calculated first within each article and then averaged across articles.  
`countA_arti`: the number of articles reporting all effects in more than half of the employed ANOVAs.  
`countA`: the number of multi-way ANOVAs. 

##### Apply corrections (per ANOVA)

Different from the previous analysis, this section analyzed the prevalence across only all ANOVAs directly.
...
df_conv_a %>%
  ...
  add_row(summarize(df_conv_a,
          ...
          .before=1) # add summary row
```
On average, 74.31% of the inspected multi-way ANOVAs reported the results of all effects (at least those statistically significant ones) (`conA`). `count` displays the number of multi-way ANOVAs. 
  
Notably, only a few of the remaining articles reported the effects of only one effect (although they did not report all).  

<br>
Whether any correction method was applied for each ANOVA:
```{r}
...
df_prev_a_corr
```
Result shows that no correction method (e.g., omnibus F-test or Bonferroni) was applied in any reviewed multi-way ANOVA.

#### Convention B: post-hoc
Convention B was only inspected in ANOVAs with at least one factor of three or more levels (and the results for that factor was reported).
```{r}
df_conv_b <- df_conv %>%
  filter(max_No_levels>2,
...
df_prev_b <- df_conv_b_artile %>%
  ...
  summarize(conB = mean(meanB_arti),
            countB_arti = sum(meanB_arti>0.5),
            countB= n()) %>%
...
`conB`: the prevalence of Convention B: the percentage of research articles performed the post-hoc analysis when the main effect (with three or more levels) was significant *or* did not perform the post-hoc analysis when the main effect was not significant. It was calculated first within each article and then averaged across articles.  
`countB_arti`: the number of articles applying convention B in more than half of the employed ANOVAs.  
`countB`: the number of ANOVAs meeting the criteria, i.e., incorporating at least one factor of three or more levels (and the results for that factor was reported).

##### Apply corrections (per ANOVA)

Different from the last analysis, this section calculates the prevalence across ANOVAs directly.
```{r}
...
df_conv_b %>%
  ...
  add_row(summarize(df_conv_b,
          ...
          .before=1) # add summary row
```
`conB` displays the prevalence of Convention B across all ANOVAs. `count` displays the number of ANOVAs meeting the criteria.

Check multiple comparison corrections only for ANOVAs where post-hoc analysis was performed:
```{r}
df_prev_b_corr <- df_conv_b %>%
  ...
  add_row(df_conv_b %>% 
            filter(b8_posthoc == 1) %>%
            group_by(journal) %>% 
            summarize(b9_correction = 'Overall', count = n()),
          .before = 1) %>% 
  mutate(Conv = "B") %>% 
  mutate(b9_correction=fct_relevel(b9_correction, "none", "LSD", "Tukey", "Bonferroni", 
                                "Holm-Bonferroni", "Duncan"),
         journal = factor(journal, levels=c("Overall", "JEPG", "Psych Sci", "J Abn Psych", "JCCP", "JESP", "JPSP")))

df_prev_b_corr %>% 
  pivot_wider(journal, names_from = b9_correction, names_sort = TRUE,
              values_from = count, values_fill = 0) %>% 
  mutate(Overall = if_else(journal=="Overall", sum(Overall), Overall)) 
```
Note: the analyses and results above included the cases when the corresponding main effect was not significant. The results for significant and non-significant main effects were following:
...
df_conv_b %>%
  ...
  mutate(b9_correction=fct_relevel(b9_correction, "none", "LSD", "Tukey", "Bonferroni", 
                                "Holm-Bonferroni", "Duncan"),
         b6_main_effect_sig = factor(b6_main_effect_sig, levels=c(1,0)),
         journal = factor(journal, levels=c("Overall", "JEPG", "Psych Sci", "J Abn Psych", "JCCP", "JESP", "JPSP"))) %>% 
  pivot_wider(journal, names_from = c(b6_main_effect_sig, b9_correction),
              names_sort = TRUE,
              values_from = count, values_fill = 0) %>% 
  arrange(journal)
```
...
Convention C was only inspected in multi-way ANOVAs (and the result for the two-way interaction was reported).  
```{r}
df_conv_c <- df_conv %>%
  filter(N_factor>1,
         c6_inter_sig %in% c("1", "0")) %>% # only include ANOVAs reporting the interaction
...
df_prev_c
```
`conC`: the prevalence of Convention C: the percentage of research articles performed the simple effect analysis when the interaction was significant *or* did not perform the simple effect analysis when the interaction was not significant. It was calculated first within each article and then averaged across articles.  
`countC_arti`: the number of articles applying convention C in more than half of the employed ANOVAs.  
`countC`: the number of ANOVAs meeting the criteria, i.e., in multi-way ANOVAs (and the result for the two-way interaction was reported).
...
df_conv_c %>%
  ...
  add_row(summarize(df_conv_c,
          ...
          .before=1) # add summary row
```
`conC` displays the prevalence of Convention B across all ANOVAs. `count` displays the number of ANOVAs meeting the criteria.
  
Check multiple comparison corrections only for ANOVAs where simple effect analysis was performed:
```{r}
df_prev_c_corr <- df_conv_c %>%
  ...
  add_row(df_conv_c %>%
            filter(c8_simple_effect == 1) %>%  # only include ANOVA performing simple 
            group_by(journal) %>%
            summarize(c9_correction = 'Overall', count = n()),
          .before = 1) %>% 
  mutate(Conv = "C") %>% 
  mutate(c9_correction=fct_relevel(c9_correction, "none", "LSD", "Tukey", "Bonferroni", "Scheffe",
                                "Holm-Bonferroni"),
         journal = factor(journal, levels=c("Overall", "JEPG", "Psych Sci", "J Abn Psych", "JCCP", "JESP", "JPSP"))) 
  
df_prev_c_corr %>% 
  pivot_wider(journal, names_from = c9_correction, names_sort = TRUE,
              values_from = count, values_fill = 0) %>% 
  mutate(Overall = if_else(journal=="Overall", sum(Overall), Overall)) 
```
Note: the analyses and results above included the cases when the corresponding interaction was not significant. The results for significant and non-significant interactions are following:
...
df_conv_c %>%
  ...
  mutate(c9_correction=fct_relevel(c9_correction, "none", "LSD", "Tukey", "Bonferroni", "Scheffe",
                                "Holm-Bonferroni"),
         c6_inter_sig = factor(c6_inter_sig, levels=c(1,0)),
         journal = factor(journal, levels=c("Overall", "JEPG", "Psych Sci", "J Abn Psych", "JCCP", "JESP", "JPSP"))) %>% 
  pivot_wider(journal, names_from = c(c6_inter_sig, c9_correction), 
              names_sort = TRUE,
              values_from = count, values_fill = 0) %>% 
  arrange(journal)
```
...
Convention D was only inspected in ANOVAs with three or more factors (and the highest-level interaction was significant).
...
df_prev_d
```
About half of the articles separated one factors and performed more ANOVAs when the highest-level interaction is significant (although there are not many articles included three or more factors). Since percentage is just around chance (50%), Convnetion D does not seem to be popular.

#### Convention E: combine 
Convention E was only inspected in ANOVAs with three or more factors (and the highest-level interaction was not significant).
...
df_prev_e
```
None of the examined research articles combined factors and perform additional ANOVA (explicitly).

#### Apply any convention 

```{r}
df_conv_any <- df_conv_a_article %>% 
  full_join(df_conv_b_artile, Joining, by = c("journal", "Paper ID")) %>% 
  full_join(df_conv_c_artile, Joining, by = c("journal", "Paper ID")) %>% 
  full_join(df_conv_d_artile, Joining, by = c("journal", "Paper ID")) %>% 
  full_join(df_conv_e_artile, Joining, by = c("journal", "Paper ID")) %>% 
  mutate(a1_per_article = if_else(is.na(a1_per_article), 0, a1_per_article)) %>% 
  mutate(meanB_arti = if_else(is.na(meanB_arti), 0, meanB_arti)) %>% 
  mutate(meanC_art = if_else(is.na(meanC_art), 0, meanC_art)) %>% 
  mutate(meanD_art = if_else(is.na(meanD_art), 0, meanD_art)) %>% 
  mutate(meanE_art = if_else(is.na(meanE_art), 0, meanE_art)) %>% 
  transmute(journal, `Paper ID`, 
            anyconv = as.integer(((a1_per_article>0.5)+ 
                                    (meanB_arti>0.5)+
                                    (meanC_art>0.5)+
                                    (meanD_art>0.5)+
                                    (meanE_art>0.5))>0))

df_prev_any <- df_conv_any %>%
  group_by(journal) %>%
  summarize(countAny = sum(anyconv)) %>%
  add_row(df_conv_any %>%
            summarize(journal = "Overall", 
                      countAny = sum(anyconv)),
          .before=1) %>%  # add summary row
  transmute(journal,
            count = countAny)
...
df_prev_con <- df_prev_a %>% 
  ...
  left_join(df_prev_any, by="journal") %>% 
...
df_prev_con
```
The numbers in the parentheses are the total count meeting the criteria/conditions. For example, the numbers in parentheses in `Conv. A` shows that 193 research articles employ more than one factor and 71.37% of them reported all the (significant) effects.


```{r}
# summary of corrections applied in Convention B, and C (no corrections were applied to A)
df_cor <- bind_rows(rename(df_prev_c_corr, correction=c9_correction),
          rename(df_prev_b_corr, correction=b9_correction)) %>% 
  mutate(correction=fct_relevel(correction, "none", "Tukey", "Bonferroni", "Scheffe", 
                                "Holm-Bonferroni", "Duncan", "LSD")) 

df_cor %>% 
  filter(journal == "Overall") %>% 
  select(-journal) %>% 
  pivot_wider(correction, names_from = "Conv", values_from = "count", values_fill = 0)
  
```

```{r}
df_cor %>% 
  pivot_wider(journal, names_from = c("Conv","correction"), values_from = count,
              values_fill = 0)
```


## Summary
This is the Table 1 in the main manuscript.
```{r}
df_prevalence %>% 
  select(Journal = journal,
         Research = In3_research,
         `ANOVA` = In4_ANOVA) %>% 
  inner_join(df_prev_con, by = "Journal") %>% 
  select(Journal, Research, ANOVA, `Conv. Any`, everything())
```
