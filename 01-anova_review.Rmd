...
#### After matching the number of ANOVAs in each article 
...
df_complex %>%
  ...
  summarize(mean_N_factor = mean(N_factor),
            mean_max_N_levels = mean(Max_level),
...
```{r fig.width=10, fig.asp=.5, warning=FALSE}
for (iN in 1:4) {
...
df_conv_a_article %>%
  group_by(journal) %>%
  summarize(conA = mean(a1_per_article), count=n()) %>%
...
```
Results showed about 71.37% research articles reported the results of all (significant) main effects and interaction in the ANOVA.
...
df_conv_b_artile %>%
  ...
  add_row(df_conv_b_artile %>%
          ...
          .before=1) # add summary row
```
Overall, about **74.61%** research articles performed the post-hoc analysis when the main effect (with three or more levels) was significant or did not perform the post-hoc analysis when the main effect was not significant.
