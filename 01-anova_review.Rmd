...
df_conv_any <- df_conv_a_article %>% 
  left_join(df_conv_b_artile, Joining, by = c("journal", "Paper ID")) %>% 
  mutate(meanB_arti = if_else(is.na(meanB_arti), 0, meanB_arti)) %>% 
  left_join(df_conv_c_artile, Joining, by = c("journal", "Paper ID")) %>% 
  mutate(meanC_art = if_else(is.na(meanC_art), 0, meanC_art)) %>% 
  left_join(df_conv_d_artile, Joining, by = c("journal", "Paper ID")) %>% 
  mutate(meanD_art = if_else(is.na(meanD_art), 0, meanD_art)) %>% 
  left_join(df_conv_e_artile, Joining, by = c("journal", "Paper ID")) %>% 
...
df_prev_any <- df_conv_any %>%
  ...
  add_row(df_conv_any %>%
          ...
          .before=1) %>%  # add summary row
  left_join(df_included, by="journal") %>% 
  transmute(journal,
            conAny = countAny/In5_include,
            count = In5_include)
...
df_prev_con <- df_prev_a %>% 
  ...
  transmute(Journal = journal,
            ...
            `Conv. Any` = paste0(round(conAny*100,2),"% (", count, ")")
            )
