...
df_prev_a <- df_conv_a_article %>%
  ...
  summarize(conA = mean(a1_per_article), 
            countA_arti = sum(a1_per_article>0.5),
            countA=n()) %>%
...
df_prev_b_corr %>% 
  ...
  mutate(Overall = if_else(b9_correction=="Overall", sum(Overall), Overall)) %>% 
  slice(2:n(),1)
```
Note: this analyses and results included ANOVAs performed post-hoc analysis and apply corrections, but the corresponding main effect was not significant. 

##### Conduct post-hoc analysis and apply corrections for significant main effects
(per ANOVA)
```{r}
df_conv_b %>%
  filter(b8_posthoc == 1) %>% # only examine articles performing post-hoc analysis
...
df_conv_any <- df_conv_a_article %>% 
  ...
  transmute(journal, `Paper ID`, 
            conv = a1_per_article+meanB_arti+meanC_art+meanD_art+meanE_art,
            anyconv = as.integer(conv>0)) 
