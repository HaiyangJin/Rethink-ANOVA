...
df_conv <- bind_rows(
  ...
  mutate(`Paper ID` = as_factor(`Paper ID`),
         ...
         c6_inter_sig = as.integer(c6_inter_sig),
...
df_complex <- df_conv %>%
  filter(included==1) %>%
  transmute(`Paper ID`, N_factor, max_No_levels, 
...
plot_design <- df_complex %>%
  ...
  facet_grid(rows=vars(N_factor), scales = "free") +
