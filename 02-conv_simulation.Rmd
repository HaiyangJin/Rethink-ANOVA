...
df_posthoc_tmp <- df_TypeI_posthoc %>% 
  ...
  mutate(adjust = if_else(effects == "TypeI_main", "uncorrected", 
         ...
         effects = fct_recode(effects, `Main effect\n` ="TypeI_main", 
                              ...
                              `Main effect & Pairwise comparisons\nwith six tests (CBER)`="TypeI_main&posthoc")) %>% 
  distinct() 
...
for (i in unique(df_posthoc_tmp$N_level)) {
  ...
  assign(
    nam, df_posthoc_tmp %>% 
      mutate(effects = fct_recode(effects, 
                                  `Pairwise comparisons\n(FWER)`="Pairwise comparisons\n(six tests, FWER)", 
                                  `Main effect & Pairwise\ncomparisons (CBER)`="Main effect & Pairwise comparisons\nwith six tests (CBER)")) %>% 
      filter(N_level == i) %>% 
      ggplot(aes(x=as.numeric(adjust), y = TypeI*100, fill=adjust)) + 
      geom_col(width=1) +   
      facet_grid(cols=vars(effects), scales="free_x", space="free_x", switch="x")+
...
df_inter_simp_tmp <- df_TypeI_inter %>% 
  ...
  mutate(
    ...
    effects = fct_recode(
      ...
      `Interaction & simple effects\nwith two tests (CBER)`="TypeI_inter&any_byA_2",
      `Interaction & simple effects\nwith four tests (CBER)`= "TypeI_inter&any_byAB_4"))
...
df_inter_simp_tmp_supp <- df_TypeI_inter %>% 
  ...
  mutate(
    ...
    effects = fct_recode(
      ...
      `Simple effect analysis\n(four tests, FWER)`="TypeI_any_simple_byAB_4",
      `Interaction & simple effects\nwith two tests (CBER)`="TypeI_inter&any_byB_2",
      `Interaction & simple effects\nwith four tests (CBER)`= "TypeI_inter&any_byAB_4"))
